...
def main(args):
    ...
    train_content_loader = get_dataloader(panda_config, 'train', panda=True)
    train_class_loader = get_dataloader(config, 'train')
    test_content_loader = get_dataloader(panda_config, 'test', panda=True)
    test_class_loader = get_dataloader(config, 'test', shuffle=True)
    trainfull_content_loader = get_dataloader(panda_config, 'trainfull', shuffle=True, panda=True)
    trainfull_class_loader = get_dataloader(config, 'trainfull', shuffle=True)
    test_rec_loader = get_dataloader(panda_config, 'test',  shuffle=True, panda=True)
    rec_loader = cycle(test_rec_loader)

    # Trainer
    trainer = Trainer(panda_config)

    tr_info = open(os.path.join(panda_config.info_dir, "info-network"), "w")
    ...
    # Summary Writer
    train_writer = SummaryWriter(os.path.join(panda_config.tb_dir, 'train'))
    test_writer = SummaryWriter(os.path.join(panda_config.tb_dir, 'test'))
    ...
    while True:
        ...
        if (iterations + 1) % panda_config.log_freq == 0:
        ...
        if ((iterations + 1) % panda_config.mt_save_iter == 0 or (
                iterations + 1) % panda_config.mt_display_iter == 0):
            if (iterations + 1) % panda_config.mt_save_iter == 0:
            ...
            with torch.no_grad():
                ...
                for phase, co_loader, cl_loader in [['trainfull', trainfull_content_loader, trainfull_class_loader],
                    ...
                    for status in ["3d"]:
                        ...
                        output_path = os.path.join(panda_config.output_dir, name)
                        print("%s saved" % name)
                        torch.save(outputs, output_path)

        if (iterations + 1) % panda_config.save_freq == 0:
